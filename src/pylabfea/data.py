...
class Data(object):
    ...
    def __init__(self, msl, path_data=None, path_json=None,
        ...
        def calc_grid_val():
            ...
            for iset, dset in enumerate(self.set):  # loop over data sets
                if not self.flow_stress:
                    ...
                    for ipeeq, peeq in enumerate(
                            self.mat_param['work_hard']):  # loop over PEEQS for which flow stresses are required
                        hs = []
                        ht = []
        ...
        print('Imported %i data sets for textures, with %i hardening stages and %i load cases each.' % (
        self.Nset, npe, Nlc_min))
        print('Averaged properties : E_av=%5.2f GPa, nu_av=%4.2f, sy_av=%4.2f MPa' % (
        self.E_av / 1000, self.nu_av, self.sy_av))
        self.mat_param = {
        ...
        self.mat_param[
            'flow_seq_av'] = sflow_av  # averaged equiv. flow stresses at PEEQs in 'work_hard' for each texture
    ...
    class Set_CSV(object):
        ...
        def __init__(self, db, name, plot=False):
                ...
                if ubc_names[0] in names:
                    ...
                    try:
                        ...
                        if lc == 'disp':
                            ...
                            eeq[ind] = 1.
                            self.ubc = self.eps / eeq[:,
                                                  None]  # generate information on load case from normalized strain
                            self.ubc[ind] = self.ubc[
                                ind + 1]  # cases with small strains inherit ubc from subsequent case
                        elif lc == 'force':
                            seq = sig_eq_j2(self.sig)
                            ind = np.nonzero(seq < 1.e-4)[0]  # filter cases with small stresses
                            seq[ind] = 1.
                            self.ubc = self.sig / seq[:,
                                                  None]  # generate information on load case from normalized stress
                            self.ubc[ind] = self.ubc[
                                ind + 1]  # cases with small stress inherit ubc from subsequent case
                        else:
            ...
            if np.amax(np.abs(self.sc_full[:, 2])) > 1.:
                ...
                warnings.warn(
                    '*** Warning: Large hydrostatic stresses: minimum p=' + h1 + ' MPa, maximum p=' + h2 + ' MPa')
            ...
            if not db.flow_stress:
                ...
                if db.mirror:
                    ...
                    sign = np.sign(
                        np.sum(hs1 * hs2, axis=1))  # filter stress components where sign has changed by mirroring
            ...
            pipeline = [
                ...
                {'$match':  # filter only those entries, where
                     {'state': 'COMPLETED',  # $fireworks.state ==  COMPLETED
                      ...
                      "launches.action.update_spec.results_json": {"$exists": "true"}
                      # and for which the "result_json" exists
                      }
                 },
                ...
                {'$project':
                    {
                        ...
                        'results_json': '$launches.action.update_spec.results_json'
                        # we need complete "results_json" from $launches.action.update_spec
                    }
                }
