...
class Data(object):
    ...
    def __init__(self, source, path_data='./',
        ...
        self.SE_data = None
        self.SPE_data = None   # Why three dicts + raw_data ???
        ...
        if isinstance(source, str):
            ...
            self.parse_data(raw_data, epl_crit, epl_start, epl_max, depl)  # add data to mat_data
        else:
    ...
    def read_data(self, Data_File):
        ...
        for key, val in data.items():
            res = val['Results']
            Sigma = [res["S11"], res["S22"], res["S33"], res["S12"], res["S13"], res["S23"]]  # Order ???
        ...
        # For having also the elastic data and shift the 0 plastic strain to 0.02% to match the micromechanical data.
        ...
        for key, dat in Final_Data.items():
            ...
            for i in range(len(EQ_Strain_Plastic)):
                ...
                temp_epl = dat['Plastic_Strain'][i] * (1. - epc / scale)
                epl.append(temp_epl)
    ...
    def parse_data(self, db, epl_crit, epl_start, epl_max, depl):
        """
        ...
        def get_elastic_coefficients(data_pairs, method='least_square', initial_guess=None):
        ...
        def find_transition_index(stress):
            ...
            ----------
            idx: (int)
            The index within the stress array where a significant transition from linear behavior accurs.
            """
            nst = len(stress)
        ...
        for key, val in db.items():
            ...
            it = find_transition_index(val["SEQ"])
            elstrain.append(val['Total_Strain'][it])
            ...
            i_ideal = np.nonzero(val['PEEQ'] < epc_lc)[0]
            print('LC: ', ct, i_ideal[-1], val['Stress'][i_ideal[-1]], val['PEEQ'][i_ideal[-1]], epc_lc)
            if len(i_ideal) < 2:
                print(
            ...
            iel = np.nonzero(val['PEEQ'] < eps_lc)[0]  # could be replaced by ipl[0] - 1
            ipl = np.nonzero(np.logical_and(val['PEEQ'] >= eps_lc, val['PEEQ'] <= epm_lc))[0]
            if len(iel) < 2:
            ...
            if len(ipl) < 2:
                print(
                    f'Skipping data set {key} (No {ct}): No plastic range: IPL: {ipl}, vals: {len(val["PEEQ"])}; '
                    f'{epl_start}, {epl_max}')
                Nlc -= 1
                continue
            # store different indices and critical valuesfor statistical analysis
            # assert iel[-1] == ipl[0] - 1  # not given for atomistic data
            ...
            sy_av += val['SEQ'][i_ideal[-1]]
            if val['PEEQ'][ipl[-1]] > peeq_max:
                peeq_max = val['PEEQ'][ipl[-1]]

            eps = -depl
            nv = 0
            for i in ipl:
                # CHANGES: A Shift in the data in order to have 0 equivalent plastic strain at start of yielding.
                # offset is eps_lc, otherwise, values might be negative ???
                hh = val['PEEQ'][i]
                if hh >= eps + depl:
                    sig.append(val['Stress'][i])
                    sc_epl = max(0., 1. - epc_lc / hh)
                    epl.append(val['Plastic_Strain'][i] * sc_epl)
                    eps = hh
                    nv += 1
            lc_ind_list[ct] = nv + lc_ind_list[ct - 1]
            """    sig.append(val['Stress'][i])
                temp_epl = val['Plastic_Strain'][i] * (1. - epc_lc / val['PEEQ'][i])
                epl.append(temp_epl)
        ...
        data_pairs = list(zip(elstrain, elstress))
        C = get_elastic_coefficients(data_pairs, method='least_square')
        sy_av /= Nlc
        self.mat_data['flow_stress'] = np.array(sig)
        self.mat_data['plastic_strain'] = np.array(epl)
        self.mat_data['lc_indices'] = lc_ind_list
        self.mat_data['peeq_max'] = peeq_max - ep_c / Nlc
        self.mat_data['epc'] = ep_c / Nlc
        self.mat_data['ep_start'] = ep_s / Nlc
        self.mat_data['ep_max'] = ep_m / Nlc
        self.mat_data['elast_const'] = C
        self.mat_data['sy_av'] = sy_av
        self.mat_data['Nlc'] = Nlc
        self.mat_data['sy_list'] = sy_list
        self.mat_data['sig_ideal'] = np.array(sig_ideal)
        self.mat_data['elstress'] = elstress  # can be removed as elastic constants are evaluated here
        self.mat_data['elstrain'] = elstrain
        self.mat_data['transition_ind'] = it_list
        print(f'\n###   Data set: {self.mat_data["Name"]}  ###')
        print(f'Type of microstructure: {Key_Translated["Texture_Type"]}')
        print(f'Estimated elastic constants (in GPa): C={C*1.E-3}')
